...
contract CompoundLender is BaseTokenizedStrategy {
        ...
        returns (uint256 _totalAssets)
    {
        if (!TokenizedStrategy.isShutdown()) {
            ...
            uint256 looseBalance = ERC20(asset).balanceOf(address(this));
            if (looseBalance > 0) {
                require(cToken.mint(looseBalance) == 0, "ctoken: mint fail");
            }
        }
    ...
    function _claimAvaxRewards() internal {
        ...
        COMPTROLLER.claimReward(1, holders, cTokens, false, true);
    }

    function _claimProtocolRewards() internal {
        CTokenI[] memory cTokens = new CTokenI[](1);
        cTokens[0] = cToken;
        address payable[] memory holders = new address payable[](1);
        holders[0] = payable(address(this));
        ...
        // Claim only rewards for lending to reduce the gas cost
        COMPTROLLER.claimReward(0, holders, cTokens, false, true);
    }
